...
function makeCache( options ) {
  ...
  function makeTemplateCache( location ) {
    ...
    var dest = path.join( _destination, location );
  ...
  function updatePackageFile( file ) {
    ...
    var dest = path.join( _destination, file.location );
  ...
  while( _locations.length > 0 ) {
    makeTemplateCachePromise(_locations.shift());
  }

  function makeTemplateCachePromise(location) {
    promisesArr.push(
      makeTemplateCache( location )
        .then(function(file) {
